...
function extractReviewsFromPage() {
  ...
  return Array.from(reviewElements).map((review) => {
    const customer = review.querySelector('.author, .reviewer-name, .customer-name, h3')?.textContent?.trim() || 'Unknown customer';
    const ratingEl = review.querySelector('.sc-8c600053-2, .rating, .stars, meter');
    let rating = 'Not rated';
    if (ratingEl) {
      // Try to extract rating from attribute or text
      rating = ratingEl.getAttribute('value') || ratingEl.textContent?.match(/Rating:\s*(\d+(\.\d+)?)\s*out of 5/i)?.[1] || ratingEl.textContent?.trim() || 'Not rated';
    }
    const date = review.querySelector('.sc-8c600053-3, .date, time, small')?.textContent?.trim() || 'Unknown date';
    const jobTitle = review.querySelector('.sc-738101b3-0, h4')?.textContent?.trim() || '';
    const content = review.querySelector('.review-content, .review-text, p')?.textContent?.trim() || '';
